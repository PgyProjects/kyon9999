...
class CreateCustomersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('customers', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            $table->increments('id');
            $table->string('wx_openid', 255)->unique()->comment('微信openid');
            ...
            $table->integer('input_zhima')->comment('手输芝麻分');
            ...
            $table->integer('type')->default('0')->comment('123对应类型A,B,C');
            $table->string('customer_id', 255)->comment('微信ID');
            ...
            $table->integer('loan_times')->nullable()->comment('借款次数');
            $table->integer('loan_sum')->nullable()->comment('借款总额');
            $table->timestamps();
        });
    }
